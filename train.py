...
def main(args):
    ...
    with tf.device(device):
        ...
        model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=args.lr),
                    ...
                    metrics=[tf.keras.metrics.BinaryAccuracy(), 'Recall', 'Precision', 'F1Score'])

        # Create save directories
        weights_save_path = f"output_focal/weights/{args.fold}"
        metrics_save_path = f"output_focal/train_metrics/{args.fold}"
